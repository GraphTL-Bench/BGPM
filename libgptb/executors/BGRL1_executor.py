...
class BGRL1Executor(AbstractExecutor):
    ...
    def _build_optimizer(self):
        ...
        if self.learner.lower() == 'adam':
            ...
            optimizer = torch.optim.AdamW(self.model.encoder_model.parameters(), lr=self.learning_rate, weight_decay=self.weight_decay)
    ...
    def train(self, train_dataloader, eval_dataloader):
        ...
        lr_scheduler = CosineDecayScheduler(self.learning_rate, 1000, self.epochs)
        mm_scheduler = CosineDecayScheduler(1 - 0.99 , 0, self.epochs)
    
        for epoch_idx in range(self._epoch_num, self.epochs):
            start_time = time.time()
            mm = 1 - mm_scheduler.get(epoch_idx+1)
            lr = lr_scheduler.get(epoch_idx+1)
            for param_group in self.optimizer.param_groups:
                param_group['lr'] = lr
            
            losses = self._train_epoch(train_dataloader, epoch_idx, self.loss_func, mm_value =  mm)
            t1 = time.time()
    ...
    def _train_epoch(self, train_dataloader, epoch_idx, loss_func=None,mm_value =0.99):
        """
        完成模型一个轮次的训练
        ...
        for data in train_dataloader:
            ...
            _, _, h1_pred, h2_pred, h1_target, h2_target = self.model.encoder_model(data.x, data.edge_index)
            ...
            loss.backward()
            self.optimizer.step()
            self.model.encoder_model.update_target_encoder(mm_value) 
