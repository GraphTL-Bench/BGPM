...
class Normalize(torch.nn.Module):
    def __init__(self, dim=None, norm='batch'):
        ...
        if norm == 'batch':
            self.norm = torch.nn.BatchNorm1d(dim, momentum=0.99)
        elif norm == 'layer':
...
class GConv(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, dropout=0.0,
        ...
        #self.batch_norm_hid = Normalize(hidden_dim, norm='layer')
        self.batch_norm = Normalize(128, norm='batch')
        
        self.layers = torch.nn.ModuleList()
        self.layers.append(GCNConv(input_dim, hidden_dim))
        for _ in range(num_layers - 1):
            self.layers.append(GCNConv(hidden_dim, 128))
...
class Encoder(torch.nn.Module):
    def __init__(self, encoder, augmentor, hidden_dim, dropout=0.0, predictor_norm='batch'):
...
class BGRL1(AbstractGCLModel):
    def __init__(self, config, data_feature):

        self.nhid = config.get('nhid', 128)
